...
function PlayTab({ user, setUser, users, saveUsers, question, submissions, setSubmissions, leaderboard, saveLB }) {
  ...
  const doAuth = () => {
    ...
    if (authMode === "register") {
      ...
      const nu = { username: form.username, email: form.email, password: form.password, streak: 0 };
  ...
  const submit = async () => {
    ...
    else newLB = [...leaderboard, { username: user.username, points: pts, correct: correct ? 1 : 0, streak, answered: 1, firstCorrects: isFirst ? 1 : 0 }];
