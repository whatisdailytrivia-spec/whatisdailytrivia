...
function AdminTab({ adminUnlocked, setAdminUnlocked, question, setQuestion }) {
  ...
  return (
    <div>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 8 }}>
              ...
              <div>
                ...
                <select style={s.input} value={qForm.points} onChange={e => setQForm({ ...qForm, points: Number(e.target.value) })}>
                  ...
                  <option value={300}>30                  <option value={300}>300 — Hard</option>
                  <option value={400}>400 — Expert</option>
                </select>
              </div>
            </div>
            <button style={{ ...s.btn, marginTop: 4 }} onClick={saveManualQ}>{saved ? "✓ Published!" : "Publish question"}</button>
          </div>
        </div>
      )}
      ...
      {section === "import" && (
        <div>
          <div style={{ color: TEXT_SEC, fontSize: "0.85rem", lineHeight: 1.6, marginBottom: 18 }}>Paste your question bank JSON below.</div>
          {bank.length > 0 && (
            <div style={{ background: "rgba(76,175,125,0.08)", border: "1px solid rgba(76,175,125,0.25)", borderRadius: 8, padding: "13px 16px", marginBottom: 14, display: "flex", alignItems: "center", justifyContent: "space-between" }}>
              <div style={{ color: "#4CAF7D", fontSize: "0.85rem", fontWeight: 500 }}>✓ {bank.length} questions loaded</div>
            </div>
          )}
          <textarea style={{ ...s.input, minHeight: 180, resize: "vertical", lineHeight: 1.5, fontFamily: "'Courier New',monospace", fontSize: "0.75rem" }}
            placeholder={'[\n  {\n    "question": "...",\n    "answer": "...",\n    "displayAnswer": "...",\n    "category": "Finance",\n    "points": 200\n  }\n]'}
            value={bankJson} onChange={e => { setBankJson(e.target.value); setBankError(""); }} />
          {bankError && <div style={{ color: "#E05C5C", fontSize: "0.82rem", marginTop: 7 }}>{bankError}</div>}
          <button style={{ ...s.btn, width: "100%", marginTop: 8 }} onClick={importBank}>
      ...
      {section === "users" && (
        <div>
          <div style={{ color: TEXT_SEC, fontSize: "0.85rem", marginBottom: 18 }}>{Object.keys(adminUsers).length} registered user{Object.keys(adminUsers).length !== 1 ? "s" : ""}. Click any row to edit.</div>
          {Object.values(adminUsers).map(u => (
            <div key={u.username}>
              <div onClick={() => { setEditingUser(editingUser === u.username ? null : u.username); setEditForm({ ...u }); setUserSaved(false); }}
                style={{ display: "grid", gridTemplateColumns: "1fr auto auto", alignItems: "center", gap: 10, padding: "11px 14px", background: editingUser === u.username ? "rgba(201,168,76,0.06)" : SURFACE, border: `1px solid ${editingUser === u.username ? "rgba(201,168,76,0.3)" : SURFACE3}`, borderRadius: editingUser === u.username ? "8px 8px 0 0" : 8, marginBottom: editingUser === u.username ? 0 : 6, cursor: "pointer" }}>
                <div>
                ...
                <div style={{ ...s.badge, background: u.state ? "rgba(201,168,76,0.1)" : SURFACE2, border: `1px solid ${u.state ? "rgba(201,168,76,0.3)" : SURFACE3}`, color: u.state ? GOLD : TEXT_MUTED }}>{u.state || "No state"}</div>
                <div style={{ ...s.mono, fontSize: "0.7rem", color: TEXT_MUTED }}>🔥{u.streak || 0}</div>
              </div>
              {editingUser === u.username && (
                <div style={{ background: "rgba(201,168,76,0.04)", border: "1px solid rgba(201,168,76,0.3)", borderTop: "none", borderRadius: "0 0 8px 8px", padding: "14px", marginBottom: 6 }}>
                  <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 8, marginBottom: 8 }}>
                    <div>
                      <div style={{ ...s.mono, fontSize: "0.65rem", color: TEXT_MUTED, marginBottom: 4, textTransform: "uppercase", letterSpacing: "0.08em" }}>Username</div>
                      <input style={s.input} value={editForm.username || ""} onChange={e => setEditForm({ ...editForm, username: e.target.value })} />
                    </div>
                    <div>
                      <div style={{ ...s.mono, fontSize: "0.65rem", color: TEXT_MUTED, marginBottom: 4, textTransform: "uppercase", letterSpacing: "0.08em" }}>State</div>
                  ...
                  <div style={{ marginBottom: 12 }}>
                    <div style={{ ...s.mono, fontSize: "0.65rem", color: TEXT_MUTED, marginBottom: 4, textTransform: "uppercase", letterSpacing: "0.08em" }}>Email</div>
                  ...
                  {adminHistories[u.username] && (() => {
                    ...
                    return (
                      <div style={{ background: SURFACE, border: `1px solid ${SURFACE3}`, borderRadius: 8, padding: "13px 15px", marginBottom: 12 }}>
                        <div style={{ ...s.mono, fontSize: "0.65rem", color: TEXT_MUTED, textTransform: "uppercase", letterSpacing: "0.1em", marginBottom: 10 }}>All-time stats</div>
                        <div style={{ display: "grid", gridTemplateColumns: "repeat(3,1fr)", gap: 7, marginBottom: 8 }}>
                          {[
                            ...
                            <div key={i} style={{ textAlign: "center" }}>
                              <div style={{ fontFamily: SERIF, fontSize: "1rem", fontWeight: 700, color: x.gold ? GOLD : OFF_WHITE }}>{x.v}</div>
                              <div style={{ ...s.mono, fontSize: "0.58rem", color: TEXT_MUTED, textTransform: "uppercase", marginTop: 2 }}>{x.l}</div>
                            </div>
                          ))}
                        </div>
                        {bestCat && <div style={{ ...s.mono, fontSize: "0.7rem", color: TEXT_SEC }}>Best category: <span style={{ color: (CAT[bestCat[0]] || CAT.Wildcard).text }}>{bestCat[0]}</span> ({Math.round((bestCat[1].correct/bestCat[1].answered)*100)}%)</div>}
                      </div>
                    );
                  })()}
